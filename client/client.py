...
def detect(opt):
    ...
    print(f'opt.output:{opt.output}')
    print(f'opt.source:{opt.source}')
    print(f'opt.yolo_weights:{opt.yolo_weights}')
    print(f'opt.deep_sort_weights:{opt.deep_sort_weights}')
    print(f'opt.show_vid:{opt.show_vid}')
    print(f'opt.save_vid:{opt.save_vid}')
    print(f'opt.save_txt:{opt.save_txt}')
    print(f'opt.img_size:{opt.img_size}')
    print(f'opt.evaluate:{opt.evaluate}')
    print(f'opt.config_deepsort:{opt.config_deepsort}')
    print(f'opt.device:{opt.device}')
    print(f'opt.augment:{opt.augment}')
    print(f'opt.conf_thres:{opt.conf_thres}')
    print(f'opt.iou_thres:{opt.iou_thres}')
    print(f'opt.classes:{opt.classes}')
    print(f'opt.agnostic_nms:{opt.agnostic_nms}')
...
if __name__ == '__main__':
    ...
    if 1:
        ...
        parser = argparse.ArgumentParser()
        args = parser.parse_args()
        args.output = 'inference/output'
        args.source = CAMERA_SRC
        args.yolo_weights = 'yolov5/weights/yolov5s.pt'
        args.deep_sort_weights = 'deep_sort_pytorch/deep_sort/deep/checkpoint/ckpt.t7'
        args.show_vid = True
        args.save_vid = False
        args.save_txt = False
        args.img_size = 640
        args.evaluate = False
        args.config_deepsort = 'deep_sort_pytorch/configs/deep_sort.yaml'
        args.device = ''
        args.augment = False
        args.conf_thres = 0.5
        args.iou_thres = 0.6
